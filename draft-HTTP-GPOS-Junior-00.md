...
# Title draft-HTTP-GPOS-JUNIOR-SANTOS-00.txt
...
## 1. Introduction
> O protocolo HTTP-GPOS é uma extensão do protocolo HTTP junto ao protocolo GPOS - protocolo que provê serviço de geolocalização, que disponilibilza longitude, latitude e altitude - que provê serviços de geolocalização de endereços IP. O protocolo é muito útil no que se diz respeito a localização da origem de requisições a um servidor, o que permite a implementação de várias soluções.
> O HTTP-GPOS pode ser utilizado para rastreamento gealocal de IP após um ataque a um servidor por exemplo.
